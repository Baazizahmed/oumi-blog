...
class Post
{
    ...
    #[Assert\Length(
        max: 100,
        maxMessage: 'Les mot clés ne doivent pas dépasser {{ limit }} caractères.',
    )]
    ...
    // REMARQUE : il ne s’agit pas d’un champ mappé de métadonnées d’entité, mais simplement d’une propriété simple.
    ...
    {
        // Récupérons les likes associés à l'article concerné, sous forme de tableau à parcourir.
        $likes = $this->getLikes()->toArray();

        // pour chaque like du tableau des likes,
        foreach ($likes as $like) {
            // Si l'utilisateur qui a déja lké l'article est le même que l'utilisateur connecté,
            if ($like->getUser() == $user) {
                // Alor retourne true pour confirmer cela.
                return true;
            }
        }

        // Dans le cas contraire, aprés avoir parcouru tout le tableau, retourne false.
        return false;
    }
}
